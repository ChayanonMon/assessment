...
public class UserLotteryController {
    ...
    public ResponseUserLotteryId buyUserLotteryByTicketId(@Pattern(regexp = "\\d{10}") @PathVariable Integer userId ,@Pattern(regexp = "\\d{6}") @PathVariable String ticketId) {
        String id = userLotteryService.buyUserLotteryByTicketId(userId, ticketId);
        return new ResponseUserLotteryId(id);
    }

    @GetMapping("/{userId}/lotteries")
    public ResponseUserLotteries getAllLotteryByUser(@Pattern(regexp = "\\d{10}") @PathVariable Integer userId) {
    ...
    public ResponseLottery deleteUserLottery(@Pattern(regexp = "\\d{10}") @PathVariable Integer userId ,@Pattern(regexp = "\\d{6}") @PathVariable String ticketId) {
